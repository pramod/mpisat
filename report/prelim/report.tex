...
Chaff~\cite{CHAFF} improves upon GRASP using two techniques.

\begin{itemize}

\item \textbf{Two-literal watching} is a data structuring technique that helps
the solver efficiently perform unit propagation and literal elimination. 

\item \textbf{Variable State Independent Decaying Sum (VSIDS)} was a decision
heuristic instroduced by Chaff which tends to make the SAT solver explore
variables that were recently involved in conflicts. The intuition behind the
metric is that it improves ``search space locality'' as variables recently
involved in conflicts are more likely to be chosen by the decision heuristic. 
This in turn improves overall performance by helping the solver ``close-out''
large regions of the search space.
\end{itemize}
